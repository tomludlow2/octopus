...
function normalizeRates(rates) {
    return rates
        .map((rate) => {
            ...
            const endMs = rate.valid_to ? new Date(rate.valid_to).getTime() : Number.MAX_SAFE_INTEGER;
...
function findRateForInterval(intervalStartIso, normalizedRates) {
    const ts = new Date(intervalStartIso).getTime();
    return normalizedRates.find((rate) => ts >= rate.startMs && ts < rate.endMs) || null;
}
...
async function upsertUsageRows(client, fuel, usageRows, normalizedRates) {
    ...
    for (const usage of usageRows) {
        ...
        const rate = findRateForInterval(startIso, normalizedRates);
...
async function importFuelRange(client, fuel, startIso, endIso, options = {}) {
    ...
    try {
        const consumptionSummary = await upsertUsageRows(client, fuel, usageRows, normalizedRates);
