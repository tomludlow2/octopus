...
async function fetchRatesForTariff(energyType, tariffCode, startIso, endIso) {
    ...
    const url = `https://api.octopus.energy/v1/products/${productCode}/${basePath}/${tariffCode}/standard-unit-rates/?period_from=${startIso}&period_to=${endIso}`;
...
async function getUnitRatesForPeriod(energyType, startIso, endIso) {
    ...
    const tariffPeriods = getTariffsForPeriod(agreements, startIso, endIso);
    ...
    const rateSets = await Promise.all(
        tariffPeriods.map((item) => fetchRatesForTariff(energyType, item.tariff_code, item.period_from, item.period_to))
    );
